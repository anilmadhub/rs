...
if(isset($_GET['s']))
{
	...
	$rdv = $activity -> retrieve('rdv','*','WHERE ref_formateur='.$_SESSION['formateur']['id'].' AND status="'.$status.'" ORDER BY DATE DESC',"$offset,10");
}
else{
	$status = 'active';
	$rdv = $activity -> retrieve('rdv','*','WHERE ref_formateur='.$_SESSION['formateur']['id'].' AND status="'.$status.'" ORDER BY DATE DESC',"$offset,10");
...
<head>
  ...
  <title>Formateur | Resaplanning</title>
...
<body class="not-front">
  ...
  <div id="container">
	...
	<?php
		$numrows = new db();
		$numrows = $numrows -> retrieve('rdv','count(*) as numrows','WHERE ref_formateur='.$_SESSION['formateur']['id'].' AND status="'.$status.'"');
