...
{	
	$login = new db();
	$result = $login -> verify('admin','identifiant ="'.$_POST['identifiant'].'" AND mot_de_passe="'.$_POST['mot_de_passe'].'"');
	if($result == "0"){
		$err_msg="Identifiant/Mot de passe incorrect";
	}
	else{
	$user = $login -> retrieve('admin','*','Where identifiant ="'.$_POST['identifiant'].'"');
	$_SESSION['admin'] = $login -> structure($user) ;
	header('location:index.php');
	}
}
?>
...
<body id='page_login'>
  ...
  <div id="container">
	...
	<p style='margin-bottom:10px;'>Espace Administrateur</p>
